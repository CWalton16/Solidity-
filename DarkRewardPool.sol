...
contract DarkRewardPool {
    ...
    uint256 public rewardHalvingRate = 6500;
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "SdoRewardPool: caller is not the operator");
        _;
    }
        ...
        if (halvingChecked) {
            ...
            if (now >= endTime) {
                ...
                if (now >= nextHalvingTime) {
                    massUpdatePools();
                    rewardPerSecond = rewardPerSecond.mul(rewardHalvingRate).div(10000); // x65% (35% decreased every week)
